...
the contract; the skill's SKILL.md is the executable procedure; the other files in `references/`
are the supporting frameworks.
...
- A2. `heldout.md` MUST contain the held-out evidence table, per-epoch hacking-gap rows and the
  judge verdict JSON of every judged epoch, and MUST carry `visibility: judge-only`.
...
- P2.6 JUDGE: runs only when visible loss = 0. Inputs: contract, epoch diff, visible + held-out
  ...
  SHOULD be a different model/provider than the implementer (default: the `second-opinion`
  skill in judge mode, headless `codex exec`, read-only sandbox; fallback: fresh Claude subagent
  labelled same-provider). The recorded verdict MUST be
  `{"met": bool, "hacking_gap": number, "reason": string}`: met only when the judge's band is
  auto-approve and visible and held-out pass rates are both 100%; hacking_gap computed by the
  lead from verifier runs. The judge MUST hunt the 7 hacking behaviors (arXiv:2606.26300) and the
  registered blind spots. A positive hacking gap with green visible evidence MUST yield met=false
  absent contrary proof in the diff.
- P2.7 DECIDE: met → Phase 3; not met → append full epoch entry (attempt summary, loss,
  gradients, verdict — never truncated **[INV-7, mistakes are signal]**) and continue. The judge
  JSON is stored verbatim in `heldout.md`; `loop.md` gets the verdict with held-out commands,
  expected outputs and assertion text abstracted away (INV-1 takes precedence over INV-7).
...
- P3.3 If a non-obvious problem was solved, it SHOULD be documented with the
...
Command (user-invoked only): `/jaiskills:goal-loop <objective> [--max-epochs N] [--patience N] [--judge codex|claude]`
