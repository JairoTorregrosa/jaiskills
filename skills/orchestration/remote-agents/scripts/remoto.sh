...
#
# Host selection (environment, all optional):
#   REMOTO_HOST           SSH host every command targets. Default: jetson.
#   REMOTO_FALLBACK_HOST  second host `hosts` probes; retarget with REMOTO_HOST=<it> when the
#                         primary is DOWN. Default: jetson-ts if REMOTO_HOST is unset, none
#                         otherwise. Set it empty to disable.
#   REMOTO_HOSTS          explicit space-separated list for `hosts` (overrides both).
# A host is any name `ssh <host>` resolves: ~/.ssh/config alias, user@ip, Tailscale name.
# Host requirements: Linux (setsid, /proc), key-based SSH (BatchMode, no prompts), bash,
# python3 (claude results), and `claude` and/or `codex` logged in on PATH (~/.local/bin is
# prepended). The local side needs ssh and rsync only; runs on bash 3.2+.
set -euo pipefail

HOST="${REMOTO_HOST:-jetson}"
if [ -n "${REMOTO_FALLBACK_HOST+x}" ]; then FALLBACK_HOST="$REMOTO_FALLBACK_HOST"
elif [ -n "${REMOTO_HOST:-}" ]; then FALLBACK_HOST=""
else FALLBACK_HOST="jetson-ts"
fi
JOBS_ROOT=".remoto/jobs"
SSH_OPTS=(-o ConnectTimeout=10 -o BatchMode=yes)

die() { echo "remoto: $*" >&2; exit 1; }
# Remote command strings are built locally on purpose; remote-side vars are \$-escaped.
...
usage: remoto.sh <command> [args]
  ...
  hosts                             probe hosts: $REMOTO_HOSTS, else $REMOTO_HOST plus
                                    $REMOTO_FALLBACK_HOST (defaults: jetson, jetson-ts)
  spawn -e claude|codex -d <dir> [-m model] [-r effort] [-n name] [--safe] <prompt|->
                                    launch a headless agent on $REMOTO_HOST; prints job id.
                                    <dir> is the REMOTE working directory (must exist).
                                    '-' reads the prompt from stdin.
                                    -m: model name; omit to use the remote CLI's default
                                    -r low|medium|high|xhigh: reasoning effort (codex only)
                                    --safe: claude acceptEdits / codex --sandbox workspace-write
                                    (default: full bypass; use only on a host you own).
...
env: REMOTO_HOST (default jetson), REMOTO_FALLBACK_HOST, REMOTO_HOSTS
EOF
}

usage() {  # usage [rc]: rc 0 -> stdout, else stderr
  if [ "${1:-1}" = 0 ]; then print_usage; else print_usage >&2; fi
  exit "${1:-1}"
}
...
hosts)
  echo "target: $HOST (REMOTO_HOST)"
  # ssh keeps the first value per option, so the shorter probe timeout goes first.
  for h in ${REMOTO_HOSTS:-$HOST $FALLBACK_HOST}; do
    if out=$(ssh -o ConnectTimeout=5 "${SSH_OPTS[@]}" "$h" \
        'export PATH="$HOME/.local/bin:$PATH"; printf "claude %s | codex %s | load %s" "$(claude --version 2>/dev/null | head -1)" "$(codex --version 2>/dev/null)" "$(cut -d" " -f1 /proc/loadavg 2>/dev/null)"' 2>/dev/null); then
      echo "$h: UP — $out"
    else
      echo "$h: DOWN"
    fi
...
spawn)
  engine="" workdir="" model="" name="agent" safe=0 effort=""
  args=()
  while [ $# -gt 0 ]; do
    case "$1" in
  ...
  if [ -n "$effort" ]; then
    [ "$engine" = codex ] || die "spawn: -r/--effort is codex-only (claude has no headless effort flag)"
    case "$effort" in low|medium|high|xhigh) ;; *) die "spawn: -r must be low|medium|high|xhigh" ;; esac
  fi
  ...
  # An unquoted ~ expands in the CALLER's shell to the local home before we ever
  # see it. That path is meaningless on the remote — refuse it instead of
  # spawning a job doomed to die at cd.
  if [ "$workdir" = "$HOME" ] || [[ "$workdir" == "$HOME/"* ]]; then
    die "spawn: -d '$workdir' is your LOCAL home path — quote the tilde ('~/...') so it expands on the remote"
  fi

  # ~ in the workdir must expand on the REMOTE, so rewrite it to a literal $HOME.
  rwd="${workdir/#\~/\$HOME}"
  rc=0; rssh "test -d \"$rwd\"" || rc=$?
  [ "$rc" = 255 ] && die "spawn: cannot reach $HOST over ssh (run 'hosts'; retarget with REMOTO_HOST=<host>)"
  [ "$rc" = 0 ] || die "spawn: remote workdir '$workdir' does not exist on $HOST"
  ...
  if [ "$engine" = claude ]; then
    ...
    # shellcheck disable=SC2016  # $JOB expands inside the generated run.sh on the remote
    agent_cmd='codex exec --skip-git-repo-check --output-last-message "$JOB/result.txt"'
    if [ "$safe" = 1 ]; then agent_cmd+=' --sandbox workspace-write'; else agent_cmd+=' --dangerously-bypass-approvals-and-sandbox'; fi
...
*) echo "remoto: unknown command '$cmd'" >&2; usage 1 ;;
esac
