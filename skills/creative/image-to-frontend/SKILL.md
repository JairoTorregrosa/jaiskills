...
description: >-
  Turn a product brief, screenshot or reference image into a working frontend page: generate 4
  visual variants with the askcodex image CLI, let the user pick one, extract a detailed build
  spec, build React/Next.js or plain HTML with the chosen image kept in context, and iterate to
  pixel-close. Also produces matching logos, hero illustrations, product shots and desktop +
  mobile mockup pairs for many ideas. Trigger on "image to frontend", "build this UI from this
  image", "make me a landing page", "design and ship this site", "turn this screenshot into
  code", "generate a mockup", "hazme una landing", "diseña y monta esta página", "convierte este
  screenshot en código", "genera mockups para esta idea", "maqueta desktop y mobile". Not for a
  standalone image with no page to build (use askcodex), not for videos (use motion-design),
  and not for auditing existing UI code against design guidelines.
---

# Image → Frontend (images via askcodex)

End-to-end workflow: brief → 4 visual variants → chosen variant → detailed build spec → real frontend code → iterate to pixel-close.

Images come from the `askcodex` CLI. Before the first image call, call the Skill tool with `askcodex`: it owns install, auth, flags, output handling and error rules. This skill only uses two command shapes: `askcodex image create "<prompt>" -o <file>.png` and `askcodex image edit "<prompt>" -i <ref>.png -o <file>.png`.

The image model is strong at photorealistic web/mobile app screenshots, accurate text rendering, world knowledge, and identity preservation across edits. But ~3 in 4 attempts fail to closely replicate a complex reference without strategy. Follow the steps; do not shortcut.
...
Fire **4 `askcodex image create` calls in parallel** (background Bash calls in one message), each with a distinct visual direction (e.g. editorial-serif, bento grid, glass-morphism, brutalist-mono). Same brief, different style. Save to `mockups/v1.png` … `mockups/v4.png`.
...
Once the visual style locks, the same image model also generates: logos, hero illustrations, product shots, supporting imagery, infographics, slide artifacts — separate `askcodex image create` / `askcodex image edit` calls. Keep style consistent by passing the chosen `v{N}.png` as a reference (`askcodex image edit "..." -i mockups/v{N}.png -o out.png`) and saying *"use the same style/palette/typography as the input image."* For motion, hand stills off to an image-to-video tool.

## Hard rules for this flow

- **One call = one image.** Never ask for "4 variants in one call"; fire 4 calls in parallel. Put composition (landscape desktop viewport, mobile portrait framing) in the prompt wording, not in flags. Exact size and transparency are not guaranteed; see `references/sizing.md`.
- **Parallelize**: 6–8 concurrent `askcodex image create` processes have worked in practice.
- **Always pass an explicit `-o` path** inside the project (`mockups/…`). Verify with `file path.png`.
- **Errors, quota, auth, reference-file limits**: follow the askcodex skill. Do not loop on a failing call.
...
- **`references/sizing.md`** — composition wording per artifact, and how to hit exact pixel sizes with a local crop/resize.
